...
import { BLUE, WHITE, GREY } from "../utils/constants";
...
export function createSearchModal(): void {
    ...
    searchForm = searchModal.querySelector(
        "#opensubtitles-search-form"
    ) as HTMLFormElement;
    cancelBtn = searchModal.querySelector(
        "#os-search-cancel-btn"
    ) as HTMLButtonElement;
    settingsBtn = searchModal.querySelector(
        "#os-settings-btn"
    ) as HTMLButtonElement;
    submitBtn = searchModal.querySelector(
        "#os-search-submit-btn"
    ) as HTMLButtonElement;
    languagesSearchInput = searchModal.querySelector(
        "#os-languages-search"
    ) as HTMLInputElement;
    languagesDropdownContainer = searchModal.querySelector(
        "#os-languages-dropdown"
    ) as HTMLElement;
    selectedLanguagesContainer = searchModal.querySelector(
        "#os-selected-languages"
    ) as HTMLElement;
    hiddenLanguagesInput = searchModal.querySelector(
        "#os-languages"
    ) as HTMLInputElement;
    searchStatusElement = searchModal.querySelector(
        "#os-search-status"
    ) as HTMLElement;
...
async function fetchLanguages(): Promise<
...
function populateLanguagesDropdown(
    ...
    const selectedCodes = input.value
        .split(",")
...
function removeLanguageTag(
    ...
    fetchLanguages().then((langs) =>
        populateLanguagesDropdown(
            langs,
            languagesSearchInput?.value.toLowerCase() || ""
        )
    );
}
...
export function hideSearchModal(): void {
    ...
    if (languagesDropdownContainer)
        languagesDropdownContainer.style.display = "none";
    // setActiveModal(ActiveModal.SEARCH);
}
...
export async function handleSearchSubmit(e: Event): Promise<void> {
    ...
    try {
        ...
        try {
            ...
            updateSearchStatus(
                `Error: ${error.message || "Failed to search subtitles"}`,
                false
            );
        }
    } catch (error) {
